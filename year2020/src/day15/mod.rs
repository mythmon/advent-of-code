    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(v) = self.starting_numbers.get(self.next_index) {
            ...
            self.last_number = Some(match self.last_seen.get(&self.last_number.unwrap()) {
                None | Some((None, None)) | Some((Some(_), None)) => 0,
        ...
        self.last_seen
            .entry(self.last_number.unwrap())
            .and_modify(|recent| {
                *recent = match *recent {
                    (Some(last), None) | (Some(_), Some(last)) => (Some(last), Some(next_index)),
                    (None, _) => unreachable!(),
                }
            })
...
impl PuzzleRunner for Part1 {
    ...
    fn cases(&self) -> Result<Vec<Box<dyn PuzzleCase>>, Box<dyn std::error::Error>> {
        Ok(GenericPuzzleCase::<Self, _, _>::build_set()
            ...
            .case("Example 6", vec![3, 1, 2], 1_836)
...
impl PuzzleRunner for Part2 {
    ...
    fn cases(&self) -> Result<Vec<Box<dyn PuzzleCase>>, Box<dyn std::error::Error>> {
        Ok(GenericPuzzleCase::<Self, _, _>::build_set()
            .case("Example 0", vec![0, 3, 6], 175_594)
            .case("Example 1", vec![1, 3, 2], 2_578)
            .case("Example 2", vec![2, 1, 3], 3_544_142)
            .case("Example 3", vec![1, 2, 3], 261_214)
            .case("Example 4", vec![2, 3, 1], 6_895_259)
            .case("Example 5", vec![3, 2, 1], 18)
            .case("Example 6", vec![3, 1, 2], 362)
            .case("Solution", vec![2, 1, 10, 11, 0, 6], 18_929_178)
            .collect())
    }
